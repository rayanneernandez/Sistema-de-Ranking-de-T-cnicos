...
const AddEditTechnicianForm: React.FC<AddEditTechnicianFormProps> = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (name.trim() === '') {
      alert('Por favor, insira o nome do técnico');
      return;
    }
    ...
    <div className="max-w-2xl mx-auto bg-gray-800 rounded-lg shadow-md overflow-hidden">
      <div className="p-4 sm:p-6 border-b border-gray-700">
        <h2 className="text-xl font-semibold text-white flex items-center">
          ...
          {isEdit ? 'Editar Técnico' : 'Adicionar Novo Técnico'}
        </h2>
      </div>
      ...
      {isEdit && (
        <div className="px-4 sm:px-6 py-3 bg-gray-700 text-xs text-gray-400">
          Nota: Os dados de métricas (atendimentos, tempos, avaliações) são atualizados na página de métricas.
        </div>
      )}
    </div>
  );
